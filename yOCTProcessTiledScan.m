...
if cropZAroundFocusArea
    ...
    zAll( ...
        ( zAll < min(zDepths) + dimOneTile.z.values(round(max(focusPositionInImageZpix(1) - cuttoffSigma*in.focusSigma,1))) ) ...
        | ...
        ( zAll > max(zDepths) + dimOneTile.z.values(round(min(focusPositionInImageZpix(end) + cuttoffSigma*in.focusSigma,length(dimOneTile.z.values)))) ) ...
        ) = []; 
